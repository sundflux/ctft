...
class cDispenser
{
    ...
    void die( Entity @inflicter, Entity @attacker )
    {
        if ( !this.inuse )
            return;
...
void dispenser_body_touch( Entity @ent, Entity @other, const Vec3 planeNormal, int surfFlags )
{
    ...
    if ( player.playerClass.tag == PLAYERCLASS_RUNNER )
    {
        ...
        other.client.inventoryGiveItem( WEAP_GUNBLADE );
...
void dispenser_body_die( Entity @self, Entity @inflicter, Entity @attacker )
{
    if ( self.count >= 0 && self.count < MAX_DISPENSERS )
        gtDispensers[self.count].die( inflicter, attacker );
}
