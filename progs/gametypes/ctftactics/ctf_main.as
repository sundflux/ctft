...
void CTF_playerKilled( Entity @target, Entity @attacker, Entity @inflicter )
{
...
void GT_PlayerRespawn( Entity @ent, int old_team, int new_team )
{
    ...
    if ( old_team != new_team )
    {
        ...
        if ( old_team == TEAM_SPECTATOR )
        {
            if ( @ent.client.getBot() != null )
            {
                ...
                ent.client.execGameCommand( "mecu \"Select class\" Grunt \"class grunt\" Medic \"class medic\" Runner \"class runner\" Engineer \"class engineer\"" );
        }
    ...
    if ( ent.isGhosting() )
    {
        if ( match.getState() == MATCH_STATE_PLAYTIME )
        {
            Entity @chaseTarget = @G_GetEntity( ent.client.chaseTarget );
            if ( @chaseTarget != null && @chaseTarget.client != null )
                ent.client.chaseCam( chaseTarget.client.name, true );
            else
    ...
    {
        // Weapons
        player.client.inventoryGiveItem( WEAP_RIOTGUN );
        player.client.inventoryGiveItem( WEAP_LASERGUN );
        player.client.inventoryGiveItem( WEAP_MACHINEGUN );
        player.client.inventoryGiveItem( WEAP_GUNBLADE );

        // Strong ammo etc
        player.client.inventoryGiveItem( AMMO_SHELLS );
        player.client.inventoryGiveItem( ARMOR_GA );

        G_PrintMsg( ent , "You're spawned as ^5RUNNER. ^7This is a the fastest class with weak health and armor\n");
        G_PrintMsg( ent , "^2classAction1: ^7Spawn ^5Invisibility\n");
        G_PrintMsg( ent , "^2classAction2: ^5Reveal nearby turrets ^7for 30 seconds.\n");
    }
    // Medic
    else if ( player.playerClass.tag == PLAYERCLASS_MEDIC )
    {
        // Weapons
        player.client.inventoryGiveItem( WEAP_PLASMAGUN );
        player.client.inventoryGiveItem( WEAP_ELECTROBOLT );
        player.client.inventoryGiveItem( WEAP_LASERGUN );
        player.client.inventoryGiveItem( WEAP_MACHINEGUN );

        // Strong ammo etc
        player.client.inventoryGiveItem( AMMO_LASERS );

        G_PrintMsg( ent , "You're spawned as ^3MEDIC. ^7This is a healer class with regeneration and ability to drop health. You can also revive dead team mates by walking over their respawner marker.\n");
        G_PrintMsg( ent , "^2classAction1: ^7Drop ^225 Health^7 for your team mates\n");
    }
    // Grunt
    else if ( player.playerClass.tag == PLAYERCLASS_GRUNT )
    {
        // Weapons
        player.client.inventoryGiveItem( WEAP_ROCKETLAUNCHER );
        player.client.inventoryGiveItem( WEAP_PLASMAGUN );
        player.client.inventoryGiveItem( WEAP_GRENADELAUNCHER );
        player.client.inventoryGiveItem( WEAP_GUNBLADE );
        // Strong ammo etc
        player.client.inventoryGiveItem( ARMOR_YA );
        player.client.inventoryGiveItem( AMMO_ROCKETS );
        player.client.inventoryGiveItem( AMMO_PLASMA );

        G_PrintMsg( ent , "You're spawned as ^1GRUNT. ^7This is a tank class with slow movement, strong armor and strong weapons\n");
        G_PrintMsg( ent , "^2classAction1: ^7Drop ^3Yellow Armor ^7for your team mates\n");
        G_PrintMsg( ent , "^2classAction2: ^7Throw a ^5Cluster Bomb^7. Press ^2classAction2^7 again to detonate it.\n");
    }
    // Engineer
    else if ( player.playerClass.tag == PLAYERCLASS_ENGINEER )
    {
        ...
        ent.client.inventoryGiveItem( AMMO_BOLTS );

        G_PrintMsg( ent , "You're spawned as ^8ENGINEER. ^7This is a supportive class with slow movement and ability to build turrets\n");
        G_PrintMsg( ent , "^2classAction1:^7 Spawn a turret with ^9Machine Gun\n");
        G_PrintMsg( ent , "^2classAction2:^7 Spawn an ^8Ammo dispenser\n");
    }
...
{
    gametype.title = "Capture The Flag: Tactics";
